...
struct edge {
    ...
    string toString() {
        string res = "{\n";
        res += "  \"start\": " + to_string(start) + ",\n";
        res += "  \"end\": " + to_string(end) + ",\n";
        res += "  \"sign\": \"" + sign + "\",\n";
        res += "  \"action\": \"" + action + "\",\n";

        // Follow tokens
        res += "  \"followTokens\": [";
        for (size_t i = 0; i < followTokens.size(); ++i) {
            res += "\"" + followTokens[i] + "\"";
            if (i < followTokens.size() - 1) res += ", ";
        }
        res += "],\n";

        // Grammer
        res += "  \"grammer\": {\n";
        res += "    \"sign\": \"" + grammer.sign + "\",\n";
        res += "    \"grammer\": [";
        for (size_t i = 0; i < grammer.grammer.size(); ++i) {
            res += "\"" + grammer.grammer[i] + "\"";
            if (i < grammer.grammer.size() - 1) res += ", ";
        }
        res += "]\n  }\n";
